...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!checkCallbackRateLimit(ip)) {
            ...
            return NextResponse.redirect(`${selfGetURL()}auth?error=rate_limit`)
        }
        ...
        if (error) {
            ...
            return NextResponse.redirect(`${selfGetURL()}auth?error=${userFriendlyError}`)
        }

        // Перевіряємо наявність authorization code
        if (!code) {
            console.error('Missing authorization code:', { ip })
            return NextResponse.redirect(`${selfGetURL()}auth?error=missing_code`)
        }
        ...
        if (authError) {
            ...
            return NextResponse.redirect(`${selfGetURL()}auth?error=${errorType}`)
        }

        // Перевіряємо чи користувач успішно автентифікований
        if (!data.user || !data.session) {
            console.error('No user or session after exchange:', { ip })
            return NextResponse.redirect(`${selfGetURL()}auth?error=no_session`)
        }
        ...
        const response = NextResponse.redirect(`${selfGetURL()}`)
        ...
        return NextResponse.redirect(`${selfGetURL()}auth?error=server_error`)
    }
}
